  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const tempS = new Date(start);
        const tempD = new Date(end);
        setStart(tempS.toLocaleDateString());
        setEnd(tempD.toLocaleDateString());
      } catch {
  ...
  return (
    <>
      {plainCard === false ? (
        ...
        <div className='card-container'>
          <div className='full'>
            <div className='job-title-full'> {jobTitle} </div>
            <div className='business'>
              <Avatar
                sx={{
                  bgcolor: blue500,
